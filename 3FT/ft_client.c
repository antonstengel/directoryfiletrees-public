...
int main(void) {
  ...
  /* our addition, never tested case */
  assert(FT_insertDir("a/d/A/b") == PARENT_CHILD_ERROR);
  assert(FT_insertFile("a/d/A/b", NULL, 0) == NOT_A_DIRECTORY);
  assert((temp = FT_toString()) != NULL);
  fprintf(stderr, "%s\n", temp);
  fflush(stderr);
  free(temp);
  ...
  assert(FT_insertDir("a/b/c//") == PARENT_CHILD_ERROR);
  /* custom tests for empty strings, three different casess */
  assert(FT_insertDir("a/b/c/J//e") == PARENT_CHILD_ERROR);
  assert(FT_insertDir("a/b/c/J//") == PARENT_CHILD_ERROR);
  assert(FT_insertFile("a/b/c//", NULL, 0) == PARENT_CHILD_ERROR);
  assert(FT_insertFile("a/b/c/J//e", NULL, 0) == PARENT_CHILD_ERROR);
  assert(FT_insertFile("a/b/c/J//", NULL, 0) == PARENT_CHILD_ERROR);
  ...
  assert(b == TRUE);
  assert(l == 10);
  assert(!strcmp(FT_replaceFileContents("A",calloc(1000,1),1000), 
  "Kernighan"));
